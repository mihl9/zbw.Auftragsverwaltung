...
namespace zbw.Auftragsverwaltung.Infrastructure.ArticleGroups.DAL
{
    ...
    {
        private const string CTE_SQL_COMMAND = "WITH CTE_ARTICLEGROUPS (Id, ArticleGroupId ) AS (SELECT	Id , ArticlegroupId FROM dbo.ArticleGroups WHERE ArticlegroupId IS NULL UNION ALL SELECT pn.Id, pn.ArticlegroupId FROM dbo.ArticleGroups AS pn INNER JOIN CTE_ARTICLEGROUPS AS p1 ON p1.Id = pn.ArticlegroupId) SELECT	Id, ArticlegroupId FROM CTE_ARTICLEGROUPS ORDER BY ArticlegroupId";
